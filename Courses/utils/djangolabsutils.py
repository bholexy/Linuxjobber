...
def grade_django_lab18(file, lab_number, current_user):
	...
	try:
		...
		driver.get("http://127.0.0.1:8000/"+app_name+'/accounts/login')
...
def grade_django_lab16(file, lab_number, current_user):
	...
	try:
		...
		import importlib
		module1 = importlib.import_module(app_name).models.GoalStatus 		#SIMILAR TO FROM APPLICATION_NAME.MODELS IMPORT GOALSTATUS
		module2 = importlib.import_module(app_name).models.ScrumyGoals 		#SIMILAR TO FROM APPLICATION_NAME.MODELS IMPORT SCRUMYGOALS


		status1 = module1.objects.get(status_name = 'Done Goal')
		status2 = module1.objects.get(status_name = 'Verify Goal')


		user = User.objects.get(email = 'louisoma@linuxjobber.com') 		#THIS ENTRY MUST ALWAYS EXIST ON THE AUTH.USER TABLE AND SHOULD NOT BE CHANGED

		goal1 = module2(goal_name = 'Test Done Goal', goal_id = 161, created_by = 'Louis', moved_by = 'Louis', owner = 'Louis', goal_status = status1, user = user)
		goal1.save()
		goal2 = module2(goal_name = 'Test Verify Goal', goal_id = 162, created_by = 'Louis', moved_by = 'Louis', owner = 'Louis', goal_status = status2, user = user)
		goal2.save()
		...
		driver.get("http://127.0.0.1:8000/"+app_name+'/home')
		...
		element = driver.find_element_by_xpath("//td[contains(text(),'louiso')]").get_attribute('innerHTML') #ISSUE WHEN STUDENT USES STATUS_ID INSTEAD OF STATUS_NAME
...
def grade_django_lab15(file, lab_number, current_user):
	...
	try:
		...
		driver.get("http://127.0.0.1:8000/"+app_name+'/movegoal/100')
...
def grade_django_lab13(file, lab_number, current_user):
	...
	try:
		...
		driver.get("http://127.0.0.1:8000/"+app_name+'/addgoal')
		driver.get("http://127.0.0.1:8000/"+app_name+'/home')
...
def grade_django_lab8(file, lab_number, current_user):
	...
	try:
		...
		check_lab7_status = ''
		check_lab7_status = GradesReport.objects.get(course_topic_id = lab8, user_id = current_user, grade = 'Passed')
	except ObjectDoesNotExist:
		check_lab7_status = ''
	except MultipleObjectsReturned:
